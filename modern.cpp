...
using ShapeType = std::variant<Circle, Rectangle, std::unique_ptr<Compound>>;

struct Compound : Shape {
  std::vector<ShapeType> children;
};

// Helper to get the base class reference from a ShapeType variant
struct BaseOf {
...
inline Shape& base(ShapeType& n) { return std::visit(BaseOf{}, n); }
...
struct Move {
  ...
  void operator()(Rectangle& r) const {
    r.x += dx;
    r.y += dy;
  }
...
inline std::ostream& operator<<(std::ostream& os, const ShapeType& n) {
...
class Canvas : public Compound {
 public:
  ...
  void place(ShapeType node) { children.push_back(std::move(node)); }
...
void benchmark() {
  ...
  std::cout << "[modern]  apply=" << us(t0, t1) << "us undo=" << us(t1, t2) << "us redo=" << us(t2, t3) << "us\n";
}
